# Documentation

Welcome to the BNB Chain AI Toolkit documentation.

Start with the guide that matches your experience level:

---

## For Everyone (No Technical Background Needed)

| Guide | Description |
|-------|-------------|
| [What Is This?](what-is-this.md) | Simple explanation of what this toolkit does and why it exists |
| [Getting Started](getting-started.md) | Step-by-step setup in under 5 minutes |
| [FAQ](faq.md) | Common questions answered simply |

## Understanding the Toolkit

| Guide | Description |
|-------|-------------|
| [Architecture](architecture.md) | How all the pieces fit together (with diagrams) |
| [AI Agents](agents.md) | Complete guide to all 72+ agents |
| [MCP Servers](mcp-servers.md) | Setting up the 6 MCP servers |

## Component Guides

| Guide | Description |
|-------|-------------|
| [Market Data](market-data.md) | Real-time prices, news, and analytics |
| [DeFi Tools](defi-tools.md) | Dust sweeping and DeFi utilities |
| [Wallets](wallets.md) | Offline wallet operations |
| [Standards](standards.md) | ERC-8004 and W3AG specifications |

## Practical Usage

| Guide | Description |
|-------|-------------|
| [Examples](examples.md) | Real-world usage patterns (beginner → advanced) |
| [Troubleshooting](troubleshooting.md) | Step-by-step problem solving |

## Resources

| Guide | Description |
|-------|-------------|
| [Awesome List](awesome.md) | Curated resources for BNB Chain + AI development |
| [Contributing](../CONTRIBUTING.md) | How to contribute to this project |
| [Security](../SECURITY.md) | Security policy and reporting |
| [Changelog](../CHANGELOG.md) | Version history |

---

## Quick Links

- [GitHub Repository](https://github.com/nirholas/bnb-chain-toolkit)
- [Report a Bug](https://github.com/nirholas/bnb-chain-toolkit/issues)
- [MIT License](../LICENSE)
